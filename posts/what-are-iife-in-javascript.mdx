...
From the things we have learned about execution context and scoping (refer to the previous blog [here](https://www.aviatecoders.com/javascript/execution-context-lexical-environment-scope-chain)) you might have noticed some issue and the problem is that whenever we define a variable we have to be careful to not repeat the same name twice or in other words avoiding namespace collisions in the global execution context
